...
public:
  ...
  retransmission_handler()
      : unacked_packets(kQueuedPackets), seq(min_seq), rtt() {}
  ...
  template <typename F>
  bool record_pkt(uint16_t tid, message *msg, F &&ctor, uint32_t &budget) {
  ...
  [[nodiscard]] uint32_t acknowledge(uint64_t seq, uint32_t budget, uint64_t now) {
    if (seq < least_unacked_pkt)
      return 0;
    stats.acked = seq;
    auto budget_update = 0u;
    update_srtt(seq, now);
    budget_update = update_budget(budget, seq);
  ...
  [[nodiscard]] uint32_t update_budget(uint16_t granted, uint64_t ack) {
    auto budget = (granted - (seq - ack - 1));
    FASTT_LOG_DEBUG("Got new capacity %lu\n", budget);
    return budget;
  }
