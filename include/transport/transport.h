...
class transport {
  ...
  struct stream {
    ...
    stream()
        : rt_handler(kOustandingMessages, kOustandingMessages),
          recv_wd(min_seq) {}
  };
...
public:
  ...
  transport(message_allocator *allocator, packet_if *pkt_sink, uint16_t sport,
      ...
      : streams(kNumStreams), target(target), allocator(allocator),
        pkt_if(pkt_sink), sport(sport) {}
  ...
  bool send_pkt(message *pkt, uint16_t tid, bool fini = false) {
    ...
    auto ctor = [&](message *pkt, uint64_t seq) {
      ...
      protocol::prepare_ft_header(pkt, seq, ack, tid, recv_wd.capacity(), fini,
                                  ts);
    };
  ...
  void open_connection() {
    ...
    bool retval = streams[0].rt_handler.record_pkt(
        0, msg, [](message *msg, uint64_t seq) {
  ...
  void accept_connection() {
    ...
    bool retval = rt_handler.record_pkt(
        0, msg, [budget = recv_wd.capacity()](message *msg, uint64_t seq) {
          protocol::prepare_init_ack_header(msg, seq, min_seq,
                                            kOustandingMessages);
        });
